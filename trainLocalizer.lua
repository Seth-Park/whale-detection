...
while (opt.epoch < opt.nEpochs) do
   ...
   if loss < opt.bestLoss then
      opt.bestAccuracy = acc
      torch.save('/nikel/dhpark/fundus_saved_weights/head_localizer/model_' .. opt.epoch .. '.t7',
                 {model=model, opt=opt})
   end
   print(string.format('SUMMARY: ' ..
              ...
              '\t test_time: %d secs' ..
              '\t best_loss: %.2f',
               opt.epoch, train_loss, test_loss,
               train_time, test_time, opt.bestLoss))
